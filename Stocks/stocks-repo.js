...
const setStockPrice = async(symbol, price, date) => {
    ...
    try {
        const results = await db.insert(existingDoc.docs[0], existingDoc.docs[0]._id);
    } catch (err) {
        console.log('update error = ', err);
    }
}
...
const _formatDates = () => {
    ...
    const formattedCurrentdDate = year + '-' + month + '-' + day + ' ' + hours + ':' + minutes + ':' + seconds;
    ...
    // Construct the desired format
    const formattedHourAgoDate = year + '-' + month + '-' + day + ' ' + hours + ':' + minutes + ':' + seconds;
